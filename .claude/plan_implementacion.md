# Plan de implementación — DataViewerIFC
...
5. Cuando un cambio afecte a documentación interna, actualizar `.claude/plan_diseño.md` al final del bloque correspondiente.
...
## Tarea 7 — Cachear `config.load()` y `modelos_disponibles()` [COMPLETADA 260511 — commit 01bfa33]

**Problema.** Llamadas redundantes a I/O:

- `ui/config_dialog.py::_aplicar()` — `_config.load()` se invoca 3 veces entre comprobación de clave y guardado.
- `ai/ollama_client.py::ensure_running()` — `self.modelos_disponibles()` se invoca 2 veces.

**Solución aplicada.** `cfg = _config.load()` se lee una vez en `_aplicar()`; `ensure_running()` guarda la lista en variable local y la recalcula solo tras pull.
...
3. Actualizar `.claude/plan_diseño.md`:
   - Nueva entrada en la sección de sesión describiendo el refactor.
   - Actualizar número de tests si cambió.
4. Actualizar `.claude/plan_diseño.md` si cambió la API pública (ToolRunner, helpers de backend).
5. Commit final opcional con `git log --oneline` para confirmar que todo está publicado.
6. Preguntar al usuario si se hace push a `origin/main`.

---

## Cierre del proyecto

- Fase de implementación completada: 260515
- Fase de prueba completada: 260515
- Último commit: 1817b51
