...
from dataviewerifc.plugins.app.herramientas import CargarIfc, EstadoApp
...
def test_cargar_nombre_con_slash():
    t = CargarIfc(MagicMock())
    assert "No se encontró" in t.execute({"nombre": "subdir/archivo.ifc"})


def test_cargar_nombre_con_backslash():
    t = CargarIfc(MagicMock())
    assert "No se encontró" in t.execute({"nombre": "sub\\archivo.ifc"})
...
def test_cargar_no_encontrado_lista_disponibles(tmp_path, monkeypatch):
    ...
    assert "buscar_archivos_ifc" in result
