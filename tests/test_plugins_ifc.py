...
from appcli.plugins.ifc.estructura import CalcularAreaTotal, ElementosDePlanta, ListarPlantas
from appcli.plugins.ifc.propiedades import ObtenerPropiedades
...
def test_buscar_elementos_con_resultados():
    ...
    with patch("appcli.plugins.ifc.buscar._q.buscar_por_tipo", return_value=[_ELEM]):
...
def test_buscar_elementos_sin_resultados():
    t = BuscarElementos(_loader())
    with patch("appcli.plugins.ifc.buscar._q.buscar_por_tipo", return_value=[]):
...
def test_contar_elementos_devuelve_numero():
    ...
    with patch("appcli.plugins.ifc.buscar._q.contar_por_tipo", return_value=7):
        resultado = t.execute({"tipo": "IfcWall"})
...
def test_contar_elementos_cero():
    t = ContarElementos(_loader())
    with patch("appcli.plugins.ifc.buscar._q.contar_por_tipo", return_value=0):
        resultado = t.execute({"tipo": "IfcBeam"})
...
def test_buscar_por_nombre_con_resultados():
    ...
    with patch("appcli.plugins.ifc.buscar._q.buscar_por_nombre", return_value=[_ELEM]):
...
def test_buscar_por_nombre_sin_resultados():
    t = BuscarPorNombre(_loader())
    with patch("appcli.plugins.ifc.buscar._q.buscar_por_nombre", return_value=[]):
...
def test_obtener_propiedades_encontrado():
    ...
    with patch("appcli.plugins.ifc.propiedades._q.obtener_propiedades", return_value=_PROPS):
...
def test_obtener_propiedades_no_encontrado():
    t = ObtenerPropiedades(_loader())
    with patch("appcli.plugins.ifc.propiedades._q.obtener_propiedades", return_value={"encontrado": False}):
...
def test_listar_plantas_con_resultados():
    ...
    with patch("appcli.plugins.ifc.estructura._q.listar_plantas", return_value=plantas):
        resultado = t.execute({})
...
def test_listar_plantas_vacio():
    t = ListarPlantas(_loader())
    with patch("appcli.plugins.ifc.estructura._q.listar_plantas", return_value=[]):
        resultado = t.execute({})
    assert "No se encontraron" in resultado
...
def test_elementos_de_planta_con_resultados():
    ...
    with patch("appcli.plugins.ifc.estructura._q.elementos_de_planta", return_value=[_ELEM]):
...
def test_calcular_area_total():
    ...
    with patch("appcli.plugins.ifc.estructura._q.calcular_area_total", return_value=42.5):
